...
- **Garage as a destination — largely SHIPPED post-spec** as the increment documented in [issue #13](https://github.com/theturpinator/ai-engineer-certification-challenge/issues/13) (2026-07-11): the garage now holds multiple cars (chat-driven car targeting, legacy profiles migrate), each with Need-for-Speed-style 0–100 stat bars plus real stock figures (LLM-generated once per car and cached), an AI-generated car portrait (gpt-image-1 via the gateway, cached server-side, regenerated when identity/color changes), a car switcher, and inline editing (PATCH/DELETE endpoints) — chat remains the primary input; forms are an override for correcting what the assistant learned.
- **Partner-vendor product sourcing — SHIPPED post-spec** as the advertiser-products increment ([issue #18](https://github.com/theturpinator/ai-engineer-certification-challenge/issues/18), 2026-07-11): the advertiser CSV is vision-ingested into a committed product catalog (7 active advertisers; placeholder/charity/giveaway campaigns ingested but never recommended), a separate mini hybrid (BM25+dense RRF) index feeds an intent-gated `recommend_products` agent tool, and product-intent chat questions stream at most two visibly **Sponsored** cards with the advertiser's creative, UTM-tracked link, and per-generation stat-delta chips for the reader's own car. Each garage car gained an **Upgrade Shop** (recommended sponsor products + a searchable catalog including unbranded generic mod categories) with "I have this" / "Add to wishlist" actions, and stat bars became dual-color dream builds: current = LLM stock baseline + committed catalog deltas of installed mods, dream = current + wishlist — deterministic, so the same product shows the same numbers on every visit. The article index, archive citations, and published evals are untouched.
